...
export default function Post({
	...
	function getTextsLength(texts: string[]) {
		let length = 0;
	...
	return (
		<div className='sketch-border dark:sketch-border-dark max-sm:w-[96%] w-[80%]  relative justify-center flex items-end'>
			...
			<div className='my-2 flex flex-col w-full'>
				<div className='max-sm:pl-2.5 pl-8 overflow-hidden space-y-1  text-left w-[95%]'>
					...
					<div
						className={`${
							isOpen ? "h-full" : "h-[min] max-h-[6rem]"
				...
				{getTextsLength(texts) > 325 && (
					<div
						...
						className='group w-32 gap-2 transition-all flex self-end cursor-pointer'
					>
						<p className='text-right'>{isOpen ? "see less" : "see more"}</p>
						<p
							className={`${
								isOpen ? "group-hover:-rotate-90" : "group-hover:rotate-90"
							} text-right  delay-75 transition-all group-hover:tracking-widest`}
						>
							{" "}
							&#62;&#62;
						</p>
